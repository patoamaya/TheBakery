...
add: async(req, res)=>{
        try{
            ...
            if (!req.files || req.files.length === 0) {
                return res.status(400).json({ message: 'Debe agregar imagenes' })
            }
...
update: async (req, res) => {
    ...
    if (!req.files || req.files.length === 0) {
      ultimateImages = existingCake.imagenes;
    } else {
      try {
            ...
            return new Promise((resolve, reject) => {
              const uploadStream = cloudinary.uploader.upload_stream(
                ...
                (err, result) => {
                  ...
                  console.log("Imagen subida con exito a Cloudinary:", result);
                  resolve({
                    url: result.secure_url,
                    public_id: result.public_id
                  });
    ...
    try {
      ...
      res.status(200).json({ message: 'Producto actualizado con exito', updatedCake: updatedCakeDoc });
    } catch (error) {
